$(function () {
    ...
    //切换模板
    $("#skinChange span").unbind("click").click(function () {
        var skin = $(this).attr("skin").replace("skin", "");
    ...
    $("#menuUl li").unbind("click").click(function () {
        if (S.stop) {
            ...
            if (data_action === "out") {
                G.alert({ content: "确认退出系统吗？",
                    ok: function () {
                        location.href =  "/out?t=" + __sysinfo.autoTid;
                        return true;
                    },
                    cancel: function () { }
                });
                ...
                for (var i = 0; i < __sysinfo.ipJoin.length; i++) {
                    table.push("<tr>");
                    table.push("<td class='w25 bc txt-right'>线路" + (i + 1) + ":</td><td class='txt-left'><input type='text' class='text-input sw120' disabled='disabled' value='响应时间:测速中'> <input type='radio' style='vertical-align:middle' name='radio' value='" + __sysinfo.ipJoin[i] + "'></td>");
                    table.push("</tr>");
                }
                table.push("</tbody></table></div>");
                var content = table.join("");
                G.alert({ title: "切换线路", content: content, width: 220, cancelVal: "测速",
                    initialize: function () {
                        SelectBoxSet(0);
                    },
                    ok: function () {
...
function middleBind(msg) {
    if (msg.data_action && S.stop) {
        ...
        if (G.query("black", "?" + msg.data_action) == 1) {//創建返回控件
            $("#shell_top").append("<span id='btn-back' title='返回' class='btn-back'></span>");
        ...
        if ((G.isAction({ key: "result", ary: actionAry }) || G.isAction({ key: "issueform", ary: actionAry }) || G.isAction({ key: "issueclass", ary: actionAry }) || G.isAction({ key: "dayclass", ary: actionAry })
            || G.isAction({ key: "myinfo", ary: actionAry }) || G.isAction({ key: "shipments", ary: actionAry }) || G.isAction({ key: "lottery", ary: actionAry }) || G.isAction({ key: "opnum", ary: actionAry })
            || G.isAction({ key: "setodds", ary: actionAry }) || G.isAction({ key: "autoodds", ary: actionAry }) || G.isAction({ key: "lmautoodds", ary: actionAry })
            || G.isAction({ key: "frommesg", ary: actionAry })
...
function onlineandnews() {
    ...
    var json =$.parseJSON('{"online":0,"list":[]}');
        if (json) {
            for (var i in json) {
                if (i != "list") {
                    $("#" + i).html(json[i]);
...
function myLmAction(id) {
    if (S.stop) {
        ...
        S.request = G.ajax("lmaction&id=" + id, function (json) {
            ...
            G.alert({ title: "单组明细", content: content, width: 450, ok: function () { return true; } });
